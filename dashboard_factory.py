...
US_TOP_TICKERS = [
    "MSFT", "AAPL", "NVDA", "GOOGL", "AMZN", "META", "TSLA", "AVGO", "LLY", "V",
    "JPM", "WMT", "XOM", "UNH", "JNJ", "ORCL", "HD", "PG", "MA", "COST",
]
...
# 섹터명 영→한 간단 매핑 (yfinance info['sector']는 영문으로만 제공됨)
_SECTOR_KO = {
    "Technology": "기술", "Communication Services": "통신서비스", "Consumer Cyclical": "임의소비재",
...
def _resolve_sector(t):
    try:
        ...
        if not sector_en:
            return None
        return _SECTOR_KO.get(sector_en, sector_en)
    except Exception:
        return None
...
KOSPI_MACRO_TICKERS = [
    ...
    ("^KQ11", "코스닥"),
    ("KRW=X", "원/달러 환율"),
    # 국채 3년물 금리: yfinance/Yahoo Finance에 신뢰할 수 있는 한국 국채 금리
    # 티커가 확인되지 않아 이번엔 제외했습니다. 무료로 쓸 수 있는 데이터 소스를
    # 찾으면(예: 한국은행 ECOS API) 추가하겠습니다.
]

KOSPI_ETF_NAMES = [
    "KODEX 200", "TIGER 코리아TOP10", "KODEX 반도체", "TIGER 반도체TOP10", "KODEX 증권",
    "KODEX 은행", "KODEX 자동차", "TIGER 2차전지", "TIGER 헬스케어", "KODEX 코스닥150",
]

KOSPI_STOCK_NAMES = [
    "삼성전자", "SK하이닉스", "LG에너지솔루션", "삼성바이오로직스", "현대차",
    "기아", "셀트리온", "KB금융", "POSCO홀딩스", "NAVER",
    "삼성SDI", "LG화학", "삼성물산", "신한지주", "하나금융지주",
    "HD현대중공업", "한화에어로스페이스", "카카오", "SK이노베이션", "POSCO퓨처엠",
]
...
def build_kospi_data(naver_id=None, naver_secret=None):
    ...
    for ticker, name in KOSPI_MACRO_TICKERS:
        ...
        if d["week52_high"] and d["week52_low"]:
            low_fmt, high_fmt = f"{d['week52_low']:,.2f}", f"{d['week52_high']:,.2f}"
        macro.append({
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# 해외 저널(WSJ·FT·블룸버그) RSS + MyMemory 번역
# ---------------------------------------------------------------------------
...
def build_us_market_data(naver_id=None, naver_secret=None):
    ...
    for ticker in US_TOP_TICKERS:
        d = yf_last_two(ticker, want_name=True, want_sector=True, want_52w=True)
        if d is None:
            log.warning(f"미국 종목 조회 실패(건너뜀): {ticker}")
            continue
        c = d["close"] - d["prev_close"]
        p = (c / d["prev_close"] * 100) if d["prev_close"] else 0
        week52_low = fmt_usd(d["week52_low"]) if d["week52_low"] else "-"
        week52_high = fmt_usd(d["week52_high"]) if d["week52_high"] else "-"
        top30.append({
            "ticker": ticker, "name": d.get("name") or ticker, "sector": d.get("sector") or "-",
...
def _dashboard_base_url():
    """GitHub Actions가 자동으로 넘겨주는 GITHUB_REPOSITORY(예: 'owner/repo-name')를
    사용해 GitHub Pages 주소를 만듭니다. 예전엔 저장소 이름을 코드에 하드코딩해서
    다른 저장소에 복사해 쓸 때마다 고쳐야 했는데, 이제는 그럴 필요가 없습니다."""
    full_repo = os.environ.get("GITHUB_REPOSITORY", "")  # "owner/repo" 형식
    if "/" in full_repo:
        owner, repo = full_repo.split("/", 1)
    else:
        owner, repo = os.environ.get("GITHUB_REPOSITORY_OWNER", "username"), "kospi-dashboard"
...
def send_cre_news_notification(token, new_articles):
    ...
    target_url = _dashboard_base_url() + "cre_news.html"
